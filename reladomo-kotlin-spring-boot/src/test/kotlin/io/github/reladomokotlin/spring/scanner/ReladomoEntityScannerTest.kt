...
class ReladomoEntityScannerTest {
    ...
    fun `test scan for entities in single package`() {
        // When
        val entities = scanner.scanForEntities(listOf("io.github.reladomokotlin.spring.scanner.test.entities"))
    ...
    fun `test scan for entities in multiple packages`() {
        ...
        val entities = scanner.scanForEntities(listOf(
            "io.github.reladomokotlin.spring.scanner.test.entities",
            "io.github.reladomokotlin.spring.scanner.test.additional"
        ))
    ...
    fun `test entity metadata extraction`() {
        ...
        assertEquals("io.github.reladomokotlin.spring.scanner.test.entities.Order", orderMetadata?.className)
    }
    ...
    fun `test entity with custom connection manager`() {
        // When
        val entities = scanner.scanForEntities(listOf("io.github.reladomokotlin.spring.scanner.test.additional"))
    ...
    fun `test scan with no entities found`() {
        // When
        val entities = scanner.scanForEntities(listOf("io.github.reladomokotlin.spring.scanner.test.empty"))
    ...
    fun `test entity scanner filters non-entity classes`() {
        // When
        val entities = scanner.scanForEntities(listOf("io.github.reladomokotlin.spring.scanner.test.mixed"))
